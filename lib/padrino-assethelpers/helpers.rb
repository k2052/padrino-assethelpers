  ...
  module AssetHelpers     
    module Helpers
      ...
      def asset_path(kind, source)
        ...
        if self.class.respond_to?(:assets_host)
          result_path = rewrite_asset_host(source)              
        elsif is_absolute  
          result_path ||= source
        else 
    ...
    private  
      def uri_root_path(source)
        ...
        File.join(ENV['RACK_BASE_URI'].to_s, root_uri || '/', source)
      end 
         
      def asset_folder_name(kind)
        if kind == :css  
          asset_folder = 'stylesheets'
          asset_folder = settings.stylesheets_folder if settings.respond_to?(:stylesheets_folder)
        elsif kind == :js                               
          asset_folder = 'javascripts'                  
          asset_folder = settings.javascripts_folder if settings.respond_to?(:javascripts_folder)                
        else
